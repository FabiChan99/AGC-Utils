...
public static class EmbedGenerator
{
    ...
    public static DiscordEmbedBuilder GetCurrentTrackEmbed(LavalinkTrack track, LavalinkGuildPlayer player)
    {
        ...
        try
        {
            eburl = BotConfig.GetConfig()["EmbedConfig"]["MusicEmbedImageURL"];
        }
        catch (Exception)
        {
            eburl = string.Empty;
        ...
        {
            eb.AddField(new DiscordEmbedField("Dauer", "``LIVE``", true));
        }
        else
        {
            eb.AddField(new DiscordEmbedField("Dauer", $"``{track.Info.Length:hh\\:mm\\:ss}``", true));
        }

        eb.AddField(new DiscordEmbedField("Lautstärke", $"``{CurrentPlayData.CurrentVolume + "%"}``", true));
        eb.AddField(new DiscordEmbedField("Queue", $"``{queuelength.ToString()}``", true));
        eb.AddField(new DiscordEmbedField("Angefordert von", requester.Mention, true));
    ...
    public static DiscordEmbedBuilder GetNowPlayingEmbed(LavalinkTrack track, LavalinkGuildPlayer player)
    {
        ...
        {
            eb.AddField(new DiscordEmbedField("Dauer", "``LIVE``", true));
            eb.AddField(new DiscordEmbedField("Aktuelle Position",
                "``LIVE``", true));
        }
        else
        {
