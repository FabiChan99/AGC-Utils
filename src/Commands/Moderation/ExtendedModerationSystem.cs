...
[EventHandler]
public class ExtendedModerationSystemEvents : BaseCommandModule
{
    [Event]
    ...
    public class ExtendedModerationSystem : ModerationSystem
    {
        ...
        public async Task MultiWarnUser(CommandContext ctx, [RemainingText] string ids_and_reason)
        {
            ...
            if (result.Result.Id == $"multiwarn_accept_{caseid}")
            {
                ...
                foreach (var user in users_to_warn_obj)
                {
                    ...
                    if (!sent)
                    {
                        await SendWarnAsChannel(ctx, user, uembed, caseid);
                    }
        ...
        private async Task SendWarnAsChannel(CommandContext ctx, DiscordUser user, DiscordEmbed uembed, string caseid)
        {
