...
namespace AGC_Management.Utils
{
    ...
    {
        public static List<TicketSearchEntry> SearchTickets = new List<TicketSearchEntry>();
        private static string FileEnding = ".html";
        ...
        public static async Task LoadTicketsIntoCache()
        {
            ...
            var completePath = Path.GetRelativePath(Directory.GetCurrentDirectory(), "tickets/transcripts");
        ...
        private static async Task ProcessAndAddFileToCache(string file)
        {
            ...
            if (SearchTickets.Any(ticket => ticket.FileHash == fileHash))
            {
                return;
            }
        ...
        /// <summary>
        /// Searches tickets by query and returns a list of matching results. "filename", "title" and "snippet" are the keys of the dictionaries.
        /// </summary>
        /// <param name="query">The query to search for.</param>
        /// <returns>A list of dictionaries containing the ticket information. Each dictionary contains the following key-value pairs: "fileName", "title", "snippet".</returns>
        public static List<Dictionary<string, string>> SearchTicketsByQuery(string query)
        {
            ...
            foreach (var ticket in SearchTickets)
            {
                ...
                MemoryStream stream = new MemoryStream(Encoding.Default.GetBytes(ticket.HtmlContent));
                doc.Load(stream);
