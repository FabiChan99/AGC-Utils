...
public partial class TogglePingCommand: ApplicationCommandsModule
{
    ...
    {
        await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("<a:loading_agc:1084157150747697203> Einstellungen werden gespeichert...").AsEphemeral());
        bool enabled = await LevelUtils.ToggleLevelUpPing(ctx.User.Id);
        if (enabled)
        {
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent("<:success:1085333481820790944> **Erfolgreich!** Levelup-Pings wurden aktiviert!"));
        }
        else
        {
