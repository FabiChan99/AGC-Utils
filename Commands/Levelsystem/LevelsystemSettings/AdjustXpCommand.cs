...
public partial class LevelSystemSettings
{
    ...
    [SlashCommand("adjust-xp", "Modifiziere den XP Stand eines Users", (long)Permissions.Administrator)]
    public static async Task AdjustXp(InteractionContext ctx,
        [Option("action", "Die auszuführende Aktion")]
        ModifyAction aktion,
        [Option("user", "Der Benutzer der bearbeitet werden soll")]
        DiscordUser user,
        [Option("menge", "XP")] int xpmenge)
    {
        ...
        if (aktion == ModifyAction.Add)
        {
            ...
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                $"<:success:1085333481820790944> **Erfolgreich!** {xpmenge} XP wurden zu {user.Username} hinzugefügt!" +
        ...
        else if (aktion == ModifyAction.Remove)
        {
            ...
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                    "<:attention:1085333468688433232> **Fehler!** Die Menge darf nicht größer als der aktuelle XP Stand sein!"));
                return;
            }

            await LevelUtils.RemoveXp(user, xpmenge);
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                $"<:success:1085333481820790944> **Erfolgreich!** {xpmenge} XP wurden von {user.Username} abgezogen. {user.Username} hat nun {currentxp - xpmenge} XP!"));
        }
        else if (aktion == ModifyAction.Set)
        {
            await LevelUtils.SetXp(user, xpmenge);
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                $"<:success:1085333481820790944> **Erfolgreich!** {user.Username} hat nun {xpmenge} XP!"));
        }
