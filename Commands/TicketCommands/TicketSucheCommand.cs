...
public class TicketSucheCommand : BaseCommandModule
{
    ...
    [Command("ticketsuche"), RequireStaffRole, Aliases("ticketsearch")]
    public async Task TicketSuche(CommandContext ctx, [RemainingText] string query)
    {
        ...
        await ctx.Message.DeleteAllReactionsAsync();
        await ctx.Client.GetInteractivity().SendPaginatedMessageAsync(ctx.Channel, ctx.User, pages, PaginationBehaviour.Ignore);
    }
}