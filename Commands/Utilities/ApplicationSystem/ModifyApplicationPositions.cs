...
public sealed class ModifyApplicationPositions : ApplicationCommandsModule
{
    ...
    public async Task AddPosition(InteractionContext ctx, [Option("position", "The position to add.")] string positionName)
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            {
                await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("Position already exists!"));
                return;
            }
        }
        await using var cmd2 = con.CreateCommand("INSERT INTO applicationcategories (positionid, positionname) VALUES (@positionid, @positionname)");
        cmd2.Parameters.AddWithValue("positionid", posId);
        cmd2.Parameters.AddWithValue("positionname", positionName);
        await cmd2.ExecuteNonQueryAsync();
        await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("Position added!"));
    ...
    public async Task RemovePosition(InteractionContext ctx, [Autocomplete(typeof(ApplicationAutocompleteProvider))] [Option("position", "The position to remove.", true)] string positionId)
    {
        ...
        if (e == 0)
        {
            await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("Position not found!"));
            return;
        }
        await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().WithContent("Position removed!"));
        ApplyPanelCommands.QueueRefreshPanel();
    }


    [SlashCommand("modifyapplystatus", "Modify an application position to be applicable.")]
    public async Task ModifyApplyStatus(InteractionContext ctx, [Autocomplete(typeof(ApplicationAutocompleteProvider))] [Option("position", "The position to modify.", true)] string positionId, [Option("status", "The status to set.")] bool status)
    {
